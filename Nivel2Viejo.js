...
BasicGame.Nivel2.prototype = {
	...
	bombPool.createMultiple(TOTAL_ENEMIES, 'bomb');
	...
	bombTextPool = this.add.group();
	// Time until explosion display.
	enemyPool.forEach(function() {
	    var text = this.add.text(0, 0, '', { font: "20px Arial", fill: "#ffffff", align: "left" }, bombTextPool);
	    text.visible = false;
	}, this);

	// Bomb on the mouse
	bombOnMouse = this.add.sprite(1000, 1000, 'bomb');
	bombOnMouse.anchor.setTo(0.5, 0.5);
	bombOnMouse.scale.setTo(0.1, 0.1);
	this.physics.enable(bombOnMouse, Phaser.Physics.ARCADE);
	...
	if (!usingBlackHole) {
	    ...
	    bombTextPool.forEach(function(display) {
	...
	if (!started && usingBlackHole && (numberOfBombs > 0)) {
	    ...
	    // bombPool.forEachDead(function(bomb) {
	    // 	console.log(bombPool.getIndex(bomb));
	    // }, this);
	    var bomb = bombPool.getFirstExists(false);
	    bomb.body.setSize(10, 10, 4, 4);
	    bomb.reset(x, y);

	    var text = bombTextPool.getAt(bombPool.getIndex(bomb));
	    text.visible = true;
	    text.x = x;
	    text.y = y;
	    numberOfBombs -= 1;
